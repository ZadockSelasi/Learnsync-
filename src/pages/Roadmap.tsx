...
export default function Roadmap() {
  ...
  const fetchRoadmaps = async () => {
    ...
    try {
      ...
      if (data.length > 0 && !activeRoadmap) {
        ...
        setCompletedMilestones(data[0].completedMilestones || []);
        setXp(data[0].xp || 0);
      }
    } catch (error) {
  ...
  const handleMilestoneComplete = async (milestoneTitle: string) => {
    if (!activeRoadmapDocId || completedMilestones.includes(milestoneTitle)) return;
    
    const newCompletedMilestones = [...completedMilestones, milestoneTitle];
    const newXp = xp + 250; // Award 250 XP per milestone
  ...
  const totalItems = (activeRoadmap?.skills?.length || 0) + (activeRoadmap?.milestones?.length || 0);
  const completedItems = completedSkills.length + completedMilestones.length;
                ...
                <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 dark:before:via-slate-700 before:to-transparent">
                  {activeRoadmap.milestones?.map((milestone: any, i: number) => {
                    ...
                    <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                      <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-white dark:border-slate-900 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 ${isCompleted ? 'bg-emerald-500 text-white' : 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400'}`}>
                        {isCompleted ? <CheckCircle2 className="w-5 h-5" /> : i + 1}
                      </div>
                      <div className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-5 rounded-2xl border shadow-sm transition-all ${isCompleted ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-800/50' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}>
                        <div className="flex items-center justify-between mb-2">
                          <h4 className={`font-bold ${isCompleted ? 'text-emerald-900 dark:text-emerald-100' : 'text-slate-900 dark:text-white'}`}>{milestone.title}</h4>
                          <span className={`text-xs font-medium px-2 py-1 rounded-full ${isCompleted ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' : 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300'}`}>{milestone.level}</span>
                        </div>
                        <p className={`text-sm mb-4 ${isCompleted ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-400'}`}>{milestone.description}</p>
                        
                        {milestone.resources && milestone.resources.length > 0 && (
                          <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700/50">
                            <h5 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Learning Resources</h5>
                            <div className="space-y-2">
                              {milestone.resources.map((res: any, idx: number) => (
                                <a key={idx} href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">
                                  {res.type === 'video' ? <PlayCircle className="w-4 h-4 shrink-0" /> : <BookOpen className="w-4 h-4 shrink-0" />}
